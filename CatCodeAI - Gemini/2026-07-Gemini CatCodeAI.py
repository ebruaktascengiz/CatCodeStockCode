...
class CategoryAIExpress:
    # CostCodeDescription bu kelimelerden birini içeriyorsa "genel/belirsiz" kabul
    # edilir ve karar verirken zayıf sinyal olarak kullanılır.
    GENERIC_KEYWORDS = {
    ...
    def __init__(self):
        ...
        # açıklama metni kullanılır (projeler arası taşınabilir bir sinyaldir).
    ...
    def reload_learning_data(self):
        ...
        CostCodeDescription -> CategoryCode dağılımını önceden hesaplar.
        """
        try:
            query = """
            ...
            # --- CostCodeDescription (normalize edilmiş) -> CategoryCode dağılımı ---
            self.costdesc_category_counts = {}
            ...
            # Hibrit Arama Metnini Vektörleştirme (CostCode NUMARASI değil, açıklaması kullanılıyor)
    ...
    def find_most_similar_examples(self, user_description: str, cost_code_description: str = None,
        ...
        """Cosine similarity ile en benzer geçmiş kayıtları arar."""
        ...
        if cost_code_description and not self._is_generic(cost_code_description):
            # Genel olmayan cost code açıklaması güçlü bir sinyaldir, tekrar ekleyerek
            # TF-IDF ağırlığını artırıyoruz.
    ...
    def _build_supporting_context(self, cost_code_description: str, mr_no: str, line_no: str = None) -> str:
        """
        Ana sinyal (tanım benzerliği) yetersiz/karasız kaldığında kullanılacak
        destekleyici bağlamı tek bir blokta toplar: CostCodeDescription dağılımı + MR paketindeki
        diğer satırlar.
        """
        lines = []
        ...
        if cost_code_description:
            ...
            if self._is_generic(cost_code_description):
                ...
                dist = ", ".join(f"{code} {cnt}/{total}" for code, cnt in counts.most_common(3))
                lines.append(f"- '{cost_code_description}' açıklamalı geçmiş kayıtlarda kullanılan kodlar: {dist}")
            else:
        ...
        if mr_no:
            ...
            if not sibling_df.empty:
                ...
                lines.append(f"- Aynı MR paketindeki diğer satırlar (çok zayıf ipucu): {items}")
    ...
    def get_smart_suggestion(self, user_description: str, cost_code_description: str = None,
        ...
        """
        Malzeme/hizmet tanımına göre en doğru kategori kodunu döndürür. Ana dayanak
        her zaman `user_description`dır; CostCodeDescription ve MR bağlamı sadece
        tanım benzerliği karasız kaldığında tie-breaker olarak kullanılır.
        """
        try:
            ...
            ANA SİNYAL - Tanım benzerliği:
            {similarity_context}

            DESTEKLEYİCİ BAĞLAM (ana sinyal karasız kaldığında tie-breaker olarak kullan, CostCode
            numarası projeye göre değiştiği için referans alınmaz, sadece açıklama metni kullanılır):
            {supporting_context}
            ...
            3. Önce ana sinyale göre karar ver. Ana sinyal iki/daha fazla kategori arasında net ayrım
               yapmana izin vermiyorsa, destekleyici bağlamı kullanarak kararını güçlendir.
            4. Hiçbir sinyal makul bir eşleşme sağlamıyorsa "BULUNAMADI" yaz.
            5. Birden fazla kod arasında net karara varamadıysan bunu saklama; GUVEN'i DUSUK işaretle
               ve GEREKCE'de hangi kodlar arasında kaldığını kısaca yaz.

            CEVABINI KESİNLİKLE şu formatta ver, başka hiçbir şey ekleme:
            KOD;GUVEN;GEREKCE
            (GUVEN: YUKSEK veya DUSUK. GEREKCE: DUSUK ise <=10 kelime, YUKSEK ise "-")

            Örnek: 3210;YUKSEK;-
            Örnek: 2110;DUSUK;Elektrik ve mekanik kod arasinda karasiz kaldim
            """
            ...
            return self._parse_model_response(raw_text, user_description)

        except Exception as e:
            return f"API-HATA: {str(e)}"

    def _parse_model_response(self, raw_text: str, user_description: str) -> str:
        """'KOD;GUVEN;GEREKCE' formatını parse eder; belirsizliği ve hataları gizlemez."""
        ...
        if len(parts) != 3:
            ...
            if candidate in self.valid_codes:
                ...
                return f"{candidate} (Format uyarısı: ham çıktı beklenmedik biçimdeydi)"
        ...
        code, guven, gerekce = parts[0].upper(), parts[1].upper(), parts[2]
        ...
        if guven == "DUSUK":
            return f"{code} (Emin değilim: {gerekce})"
        return code
...
if __name__ == "__main__":
    ...
    test_metin = "silecek suyu" ##kablo tavası,kelepce ama boru icin, silecek suyu
