...
# =
#  SSL hatası alındıgı için, gecici ekleme: GÜVENLİK DUVARI VE SSL ENGELEME YAMASI (MONKEY-PATCH)
...
class CategoryAIExpress:
    ...
    # %90 ve üzeri "yüksek güven" kabul edilir kullanıcıya kod gosterilir
    ...
    def reload_learning_data(self):
        ...
        try:
            ...
            if self.df_verified.empty:
                print("⚠SQL'de Verified=1 olan kayıt bulunamadı, önce verileri yükleyin.")
                return
            ...
            # --- CostCodeDescription (normalize) -> CategoryCode  ---
            self.costdesc_category_counts = {}
    ...
    def _build_supporting_context(self, cost_code_description: str, mr_no: str, line_no: str = None) -> str:
        """
        İKİNCİL arama: CostCodeDescription bazlı yüzdesel kod dağılımı + MR paket bağlamı.
    ...
    def get_smart_suggestion(self, user_description: str, cost_code_description: str = None,
        ...
        try:
            ...
            if not raw_text:
                ...
                print(f"⚠ Model boş cevap döndürdü. finish_reason={finish_reason}, "
    ...
    def _parse_model_response(self, raw_text: str, user_description: str,
        ...
        if len(parts) != 2:
            ...
            if candidate in self.valid_codes:
                print(f"Beklenmeyen format ama geçerli kod bulundu: '{first_line}'")
                return f"{candidate} (Format uyarısı: yüzde bilgisi alınamadı)"
            print(f" Beklenmeyen model formatı: '{raw_text}' (tanım='{user_description}')")
        ...
        if self.valid_codes and code not in self.valid_codes:
            print(f" Geçersiz kod önerildi: '{code}' (tanım='{user_description}')")
        ...
        if signal_codes and code not in signal_codes:
            print(f" Model, benzerlik/costcode sinyallerinde görünmeyen bir kod önerdi: "
...
if __name__ == "__main__":
    ...
    test_metin = "su ama silecekler için"
