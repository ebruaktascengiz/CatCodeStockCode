...
# DEĞİŞİKLİK: Dosya yolu ve uzantısı .xlsx olarak güncellendi
...
def preprocess_and_load_data(file_path):
    ...
    try:
        # DEĞİŞİKLİK: pd.read_csv yerine pd.read_excel kullanıldı
...
def add_new_example(description, code):
    """
    Yeni ve doğru bir örneği Excel dosyasına ve hafızadaki DataFrame'e ekler.
    """
    ...
    # 1. Anında öğrenme için hafızadaki DataFrame'e ekle
    df_global = pd.concat([df_global, new_row], ignore_index=True)

    # 2. Kalıcı olarak Excel dosyasına güncellenmiş DataFrame'i yaz
    # Excel'e satır eklemenin (append) standart yolu, tüm dosyayı yeniden yazmaktır.
    try:
        # DEĞİŞİKLİK: .to_csv yerine .to_excel kullanıldı. index=False önemlidir.
        df_global.to_excel(xlsx_file_path, index=False, engine='openpyxl')
    except Exception as e:
        print(f"HATA: Excel dosyasına yazılırken sorun oluştu: {e}")
        # Hata durumunda, hafızadaki son eklemeyi geri alabiliriz.
...
def find_most_similar_examples(user_description: str, top_n: int = 5):
    ...
    if cosine_similarities[most_similar_indices[0]] < 0.05:
        return pd.DataFrame()
...
if __name__ == "__main__":
    # DEĞİŞİKLİK: Güncellenmiş dosya yolu değişkeni kullanıldı
